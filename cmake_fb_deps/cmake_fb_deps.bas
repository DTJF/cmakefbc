...
Welcome to *cmake_fb_deps* tool,

- a dependency scanner for FreeBASIC source code,
- designed to be used with CMake, extended by the cmakefbc package.

This cmake_fb_deps tool is designed to be used in combination with the
...
The cmake_fb_deps tool scans one or more FreeBASIC source code files and
...
ADD_EXECUTABLE(MyProject ${BAS_SRC})
ADD_Fbc_SRC_DEPS(MyProject)
~~~

By default the output file is placed in the related *CMakeFiles* folder
and named after the target with extension `_deps.cmake` (out-of-soucre
build). The file gets removed by a `make clean` command.

The tool doesn't support conditional compiling yet. All included header
dependencies get specified, if the header file is reachable form the
given path information. But cmake_fb_deps does neither check the FreeBASIC
default header directories nor additional include directories specified
...
The source code of this cmake_fb_deps tool is free software; you can
